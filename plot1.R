...
# Sum the data for analysis
NEI_pivot <- ddply(NEI, .(year), summarize, emission_million = sum(Emissions, na.rm=TRUE))
NEI_pivot$emission_million <- NEI_pivot$emission_million/1000000.0



# Plot the result
png("plot1.png")
plot(NEI_pivot, type="o", col="blue", xlab="year", ylab="PM2.5 Emissions (million tons)",main="US PM2.5 Emissions by Year")
dev.off()
